# DRON: Docker Registry over NDN

DRON is a proof-of-concept implementation that runs a subset of [Docker Registry HTTP API](https://distribution.github.io/distribution/spec/api/) over Named Data Networking (NDN).
...
5. Start service: `pm2 start --name dron-server --restart-delay 10000 --cwd server main.js`
...
```bash
docker pull docker.yoursunny.dev/dron-server

docker build -t docker.yoursunny.dev/dron-server -f server.Dockerfile .
```
...
```bash
./dron-client --upstream https://docker.example.com --name /example/docker
```
...
```bash
docker pull docker.yoursunny.dev/dron-client

docker build -t docker.yoursunny.dev/dron-client -f client.Dockerfile .
```

Start the client container:

```bash
docker run -d --name dron-client \
  --mount type=bind,source=/run/nfd/nfd.sock,target=/run/nfd/nfd.sock,readonly=true \
  docker.yoursunny.dev/dron-client \
